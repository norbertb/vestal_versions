module VestalVersions
  ...
  module VersionTagging
    ...
    module InstanceMethods
      ...
      # the current version iteration of the parent object.
      #
      ...
      #
      # Version records corresponding to version iteration 1 are not typically created, but one will
      # be built to house the given tag if the parent object's current version iteration is 1.
      def tag_version(tag)
        v = versions.at(version) || versions.build(:iteration => 1)
        v.tag!(tag)
      end
    end
